...
function deleteProperty(sourceKeys, deleteKeys) {
  if (!deleteKeys.length || !sourceKeys.length)
    return;
...
function indexOf(source, search, fromIndex = 0) {
  ...
  if (typeof search === "string") {
    ...
    const index2 = source.indexOf(search, start);
    return index2 === -1 ? { index: -1, length: 0 } : { index: index2, length: search.length };
  }
...
const _IFrameBlock = class {
  ...
  render(blockItemContainer) {
    ...
    iframe.sandbox.add(...(iframeBlock == null ? void 0 : iframeBlock.sandbox) || _IFrameBlock.sandbox);
    iframe.setAttribute("allow", [(iframeBlock == null ? void 0 : iframeBlock.allow) || _IFrameBlock.allow].join(" "));
    iframe.style.border = "none";
    iframe.style.width = "100%";
    iframe.style.height = "100%";
    if (iframeBlock == null ? void 0 : iframeBlock.src) {
      iframe.src = iframeBlock.src;
    } else if (iframeBlock == null ? void 0 : iframeBlock.srcdoc) {
...
__publicField(IFrameBlock, "allow", ["fullscreen"]);
class ImageParticle {
  constructor(draw) {
  ...
  getOriginalMainImageList() {
    const imageList = [];
    const getImageList = (elementList) => {
...
function mergeOption(options = {}) {
  ...
  const whiteSpaceOptions = __spreadValues(__spreadValues({}, defaultWhiteSpaceOption), options.whiteSpace);
  ...
  const modeRuleOption = {
    ...
    form: __spreadValues(__spreadValues({}, defaultModeRuleOption.form), (_c = options.modeRule) == null ? void 0 : _c.form)
  };
...
function formatElementContext(sourceElementList, formatElementList2, anchorIndex, options) {
  ...
  if (!copyElement)
    return;
  const { isBreakWhenWrap = false, editorOptions, ignoreContextKeys = [] } = options || {};
  ...
  for (let e = 0; e < formatElementList2.length; e++) {
    ...
    if (isBreakWarped || !copyElement.listId && targetElement.type === ElementType.LIST) {
      ...
      deleteProperty(cloneAttr2, ignoreContextKeys);
...
function convertElementToDom(element, options) {
  ...
  if (element.underline) {
    ...
    dom.style.textDecorationStyle = ((_a = element.textDecoration) == null ? void 0 : _a.style) || "solid";
  }
    ...
    for (let e = 0; e < payload.length; e++) {
      ...
      if (element.type === ElementType.TABLE) {
        ...
        if (((_e = element.block) == null ? void 0 : _e.type) === BlockType.VIDEO) {
          ...
          const { src, srcdoc, sandbox, allow } = element.block.iframeBlock || {};
          if (src || srcdoc) {
            const iframe = document.createElement("iframe");
        ...
        if (element.type === ElementType.DATE) {
          text = ((_l = element.valueList) == null ? void 0 : _l.map((v) => v.value).join("")) || "";
        } else {
          text = element.value;
        }
        if (!text)
      ...
      for (let n = 0; n < childNodes.length; n++) {
        ...
        if (node.nodeName === "BR") {
          ...
          tableElement.querySelectorAll("tr").forEach((trElement) => {
            const trHeightStr = Number(window.getComputedStyle(trElement).height.replace("px", ""));
            const tr = {
              height: trHeightStr,
...
async function writeClipboardItem(text, html, elementList) {
...
async function writeElementList(elementList, options) {
...
class Cursor {
  constructor(draw, canvasEvent) {
    ...
    __publicField(this, "hitLineStartIndex");
    this.draw = draw;
    this.container = draw.getContainer();
...
function mouseup(evt, host) {
  ...
  if (host.isAllowDrop) {
    ...
    if (cacheEndElement.controlId && cacheEndElement.controlComponent !== ControlComponent.POSTFIX) {
      ...
      if (cachePositionContext == null ? void 0 : cachePositionContext.isTable) {
        const { tableId, trIndex, tdIndex } = cachePositionContext;
        const originElementList = draw.getOriginalElementList();
        isTdElementDeletable = !originElementList.some((el) => {
          var _a2, _b2, _c2, _d2;
          return el.id === tableId && ((_d2 = (_c2 = (_b2 = (_a2 = el == null ? void 0 : el.trList) == null ? void 0 : _a2[trIndex]) == null ? void 0 : _b2.tdList) == null ? void 0 : _c2[tdIndex]) == null ? void 0 : _d2.deletable) === false;
        });
      }
    ...
    if (((_e = host.cacheRange) == null ? void 0 : _e.startIndex) !== ((_f = host.cacheRange) == null ? void 0 : _f.endIndex)) {
      host.mousedown(evt);
    }
  }
}
...
function right(evt, host) {
  ...
  const newElementList = draw.getElementList();
  anchorStartIndex = getNonHideElementIndex(newElementList, anchorStartIndex, LocationPosition.AFTER);
  anchorEndIndex = getNonHideElementIndex(newElementList, anchorEndIndex, LocationPosition.AFTER);
  rangeManager.setRange(anchorStartIndex, anchorEndIndex);
  const isAnchorCollapsed = anchorStartIndex === anchorEndIndex;
  draw.render({
    curIndex: isAnchorCollapsed ? anchorStartIndex : void 0,
...
function tab(evt, host) {
  ...
  if (activeControl && control.getIsRangeWithinControl()) {
    ...
    const copyStyle = anchorStyle ? pickObject(anchorStyle, EDITOR_ELEMENT_STYLE_ATTR) : null;
    const tabElement = __spreadProps(__spreadValues({}, copyStyle), {
    ...
    if (isDesignMode || !((_a2 = copyElement.title) == null ? void 0 : _a2.disabled) && !((_b2 = copyElement.control) == null ? void 0 : _b2.disabled)) {
      ...
      if (defaultStyle || copyElement.type === TAB) {
        EDITOR_ELEMENT_STYLE_ATTR.forEach((attr) => {
          const value2 = (defaultStyle == null ? void 0 : defaultStyle[attr]) || copyElement[attr];
          if (value2 !== void 0) {
            newElement[attr] = value2;
          }
        });
...
async function copy(host, options) {
  const draw = host.getDraw();
...
function dragover(evt, host) {
  ...
  const cursor = draw.getCursor();
  const { cursor: { dragColor, dragWidth, dragFloatImageDisabled } } = draw.getOptions();
  if (dragFloatImageDisabled) {
    const dragElement = (_a = host.cacheElementList) == null ? void 0 : _a[host.cacheRange.startIndex];
    if ((dragElement == null ? void 0 : dragElement.type) === ElementType.IMAGE && (dragElement.imgDisplay === ImageDisplay.FLOAT_TOP || dragElement.imgDisplay === ImageDisplay.FLOAT_BOTTOM || dragElement.imgDisplay === ImageDisplay.SURROUND)) {
      return;
    }
  }
  cursor.drawCursor({
...
class GlobalEvent {
  constructor(draw, canvasEvent) {
    ...
    __publicField(this, "setPageScale", (evt) => {
      if (this.options.shortcutDisableKeys.includes(INTERNAL_SHORTCUT_KEY.PAGE_SCALE)) {
        return;
      }
      if (!evt.ctrlKey)
        return;
          ...
          for (let t = 0; t < element.trList.length; t++) {
            ...
            for (let d = 0; d < tr.tdList.length; d++) {
              ...
              const drawRowResult = this.computePageRowPosition({
                ...
                startRowIndex: 0,
                startIndex: 0,
                startX: (td.x + tdPadding[3]) * scale + tablePreX + (element.translateX || 0) * scale,
  ...
  getPositionByXY(payload) {
    ...
    for (let j = 0; j < positionList.length; j++) {
      ...
      if (leftTop[0] - left2 <= x && rightTop[0] >= x && leftTop[1] <= y && leftBottom[1] >= y) {
        ...
        if (element.type === ElementType.TAB && element.listStyle === ListStyle.CHECKBOX) {
          let index22 = curPositionIndex2 - 1;
          while (index22 > 0) {
            const element2 = elementList[index22];
            if (element2.value === ZERO && element2.listStyle === ListStyle.CHECKBOX) {
              break;
            }
            index22--;
          }
  ...
  getFloatPositionByXY(payload) {
    ...
    for (let f = 0; f < this.floatPositionList.length; f++) {
      ...
      if (currentPageNo === pageNo && element.type === ElementType.IMAGE && element.imgDisplay && payload.imgDisplays.includes(element.imgDisplay) && (!floatElementZone || floatElementZone === currentZone)) {
        ...
        if (x >= imgFloatPositionX && x <= imgFloatPositionX + elementWidth && y >= imgFloatPositionY && y <= imgFloatPositionY + elementHeight) {
          if (isTable) {
            return {
              index: index2,
              isDirectHit: true,
...
class Search {
  ...
  setSearchKeyword(payload, options) {
  ...
  getMatchList(payload, originalElementList) {
    const { isRegEnable = false, isIgnoreCase = true } = this.searchOptions || {};
    ...
    function searchClosure(payload2, type, elementList, restArgs) {
      if (!payload2)
        return;
      let text = elementList.map((e) => {
        var _a, _b;
        return !e.type || TEXTLIKE_ELEMENT_TYPE.includes(e.type) && e.controlComponent !== ControlComponent.CHECKBOX && e.controlComponent !== ControlComponent.RADIO && !e.hide && !((_a = e.control) == null ? void 0 : _a.hide) && !((_b = e.area) == null ? void 0 : _b.hide) ? e.value : ZERO;
      }).filter(Boolean).join("");
      if (isIgnoreCase) {
        text = text.toLocaleLowerCase();
      }
      const matchList = [];
      const searchStr = isRegEnable ? new RegExp(payload2) : payload2;
      let { index: index2, length } = indexOf(text, searchStr);
      while (index2 !== -1 && length !== 0) {
        matchList.push({
          index: index2,
          length
        });
        const matchResult = indexOf(text, searchStr, index2 + length);
        index2 = matchResult.index;
      ...
      for (let m = 0; m < matchList.length; m++) {
        ...
        for (let i2 = 0; i2 < matchLength; i2++) {
  ...
  compute(payload) {
    var _a;
    const isPickSelectionElementList = ((_a = this.searchOptions) == null ? void 0 : _a.isLimitSelection) && !this.range.getIsCollapsed();
    const searchElementList = isPickSelectionElementList ? this.range.getSelectionElementList() : this.draw.getOriginalElementList();
    if (!(searchElementList == null ? void 0 : searchElementList.length))
      return;
    this.searchMatchList = this.getMatchList(payload, searchElementList);
    if (!isPickSelectionElementList || !this.searchMatchList.length)
      return;
...
class PageNumber {
  ...
  static formatNumberPlaceholder(text, pageNo, replaceReg, numberType) {
    const pageNoText = numberType === NumberType.CHINESE ? convertNumberToChinese(pageNo) : `${pageNo}`;
  ...
  render(ctx, pageNo) {
    ...
    if (pageNoReg.test(text)) {
      text = PageNumber.formatNumberPlaceholder(text, pageNo + startPageNo - fromPageNo, pageNoReg, numberType);
    }
    const pageCountReg = new RegExp(FORMAT_PLACEHOLDER.PAGE_COUNT);
    if (pageCountReg.test(text)) {
      text = PageNumber.formatNumberPlaceholder(text, this.draw.getPageCount() - fromPageNo, pageCountReg, numberType);
    }
    const width = this.draw.getWidth();
    const height = this.draw.getHeight();
...
class TableTool {
  constructor(draw) {
    ...
    __publicField(this, "TABLE_SELECT_OFFSET", 20);
    __publicField(this, "draw");
    ...
    __publicField(this, "toolTableSelectBtn");
  ...
  render() {
    ...
    if (element.tableToolDisabled && !this.draw.isDesignMode())
      return;
    const { scale, table: { overflow } } = this.options;
    ...
    for (let r = 0; r < trList.length; r++) {
      ...
      for (let d = 0; d < tr.tdList.length; d++) {
        ...
        if (overflow && td2.colIndex === 0) {
          const colBorder2 = document.createElement("div");
          colBorder2.classList.add(`${EDITOR_PREFIX}-table-tool__border__col`);
          colBorder2.style.width = `${this.BORDER_VALUE}px`;
          colBorder2.style.height = `${td2.height * scale}px`;
          colBorder2.style.top = `${td2.y * scale}px`;
          colBorder2.style.left = `${td2.x * scale - this.BORDER_VALUE / 2}px`;
          colBorder2.onmousedown = (evt) => {
            this._mousedown({
              evt,
              element,
              index: 0,
          ...
          borderContainer.appendChild(colBorder2);
        }
      }
    }
  ...
  _mousedown(payload) {
    const { evt, index: index2, order, element, isLeftStartBorder } = payload;
      ...
      if (order === TableOrder.ROW) {
        ...
        if (colgroup && dx) {
          if (overflow && isLeftStartBorder) {
            if (colgroup[index2].width - dx / scale <= this.MIN_TD_WIDTH) {
              dx = (colgroup[index2].width - this.MIN_TD_WIDTH) * scale;
            }
            ...
            if (!overflow && index2 === colgroup.length - 1) {
...
class Header {
  ...
  render(ctx, pageNo) {
    ctx.save();
    ctx.globalAlpha = this.zone.isHeaderActive() ? 1 : this.options.header.inactiveAlpha;
    const innerWidth = this.draw.getInnerWidth();
    const maxHeight = this.getMaxHeight();
    const rowList = [];
    let curRowHeight = 0;
...
class Watermark {
  ...
  renderText(ctx, pageNo) {
    const { watermark: { data: data2, opacity, font, size, color, repeat, gap, numberType }, scale } = this.options;
    const width = this.draw.getWidth();
    const height = this.draw.getHeight();
    ctx.save();
    ...
    const pageNoReg = new RegExp(FORMAT_PLACEHOLDER.PAGE_NO);
    if (pageNoReg.test(text)) {
      text = PageNumber.formatNumberPlaceholder(text, pageNo + 1, pageNoReg, numberType);
    }
    const pageCountReg = new RegExp(FORMAT_PLACEHOLDER.PAGE_COUNT);
    if (pageCountReg.test(text)) {
      text = PageNumber.formatNumberPlaceholder(text, this.draw.getPageCount(), pageCountReg, numberType);
    }
    ...
    if (repeat) {
      ...
      temporaryCtx.fillText(text, (patternWidth - textWidth) / 2, (patternHeight - textHeight) / 2 + measureText.actualBoundingBoxAscent);
      ...
      ctx.fillText(text, -measureText.width / 2, measureText.actualBoundingBoxAscent - size * scale / 2);
    }
    ctx.restore();
  }
  renderImage(ctx) {
    const { watermark: { width, height, data: data2, opacity, repeat, gap }, scale } = this.options;
    if (!this.imageCache.has(data2)) {
      const img = new Image();
      img.setAttribute("crossOrigin", "Anonymous");
      img.src = data2;
      img.onload = () => {
        this.imageCache.set(data2, img);
        this.draw.render({
          isCompute: false,
          isSubmitHistory: false
        });
    ...
    if (repeat) {
      ...
      const temporaryCanvas = document.createElement("canvas");
      const temporaryCtx = temporaryCanvas.getContext("2d");
      const diagonalLength = Math.sqrt(Math.pow(imageWidth, 2) + Math.pow(imageHeight, 2));
      const patternWidth = diagonalLength + 2 * gap[0] * scale;
      const patternHeight = diagonalLength + 2 * gap[1] * scale;
      temporaryCanvas.width = patternWidth;
      temporaryCanvas.height = patternHeight;
      ...
      temporaryCtx.rotate(-45 * Math.PI / 180);
      temporaryCtx.translate(-patternWidth / 2, -patternHeight / 2);
      temporaryCtx.drawImage(this.imageCache.get(data2), (patternWidth - imageWidth) / 2, (patternHeight - imageHeight) / 2, imageWidth, imageHeight);
      const pattern = ctx.createPattern(temporaryCanvas, "repeat");
      if (pattern) {
        ctx.fillStyle = pattern;
      ...
      ctx.translate(x, y);
      ctx.rotate(-45 * Math.PI / 180);
      ctx.drawImage(this.imageCache.get(data2), -imageWidth / 2, -imageHeight / 2, imageWidth, imageHeight);
    }
    ctx.restore();
  }
  render(ctx, pageNo) {
...
class ControlSearch {
  constructor(control) {
    ...
    __publicField(this, "control");
    __publicField(this, "options");
    __publicField(this, "highlightList");
    __publicField(this, "highlightMatchResult");
  ...
  getControlHighlight(elementList, index2) {
    var _a, _b;
    const { control: { activeBackgroundColor, disabledBackgroundColor, existValueBackgroundColor, noValueBackgroundColor } } = this.options;
    const element = elementList[index2];
    ...
    if (!element.highlight) {
      isActiveControlHighlight = !isPrintMode && !!activeBackgroundColor && !!activeControlElement && element.controlId === activeControlElement.controlId && !this.control.getIsRangeInPostfix();
    }
    if (!isActiveControlHighlight) {
      isDisabledControlHighlight = !isPrintMode && !!disabledBackgroundColor && !!((_b = element.control) == null ? void 0 : _b.disabled);
    }
    if (!isDisabledControlHighlight) {
      isExitsValueControlHighlight = !isPrintMode && !!existValueBackgroundColor && !!element.controlId && this.control.getIsExistValueByElementListIndex(elementList, index2);
    }
    if (!isExitsValueControlHighlight) {
      isNoValueControlHighlight = !isPrintMode && !!noValueBackgroundColor && !!element.controlId && !this.control.getIsExistValueByElementListIndex(elementList, index2);
    }
    return (isActiveControlHighlight ? activeBackgroundColor : "") || (isDisabledControlHighlight ? disabledBackgroundColor : "") || (isExitsValueControlHighlight ? existValueBackgroundColor : "") || (isNoValueControlHighlight ? noValueBackgroundColor : "");
  }
    ...
    const computeHighlight = (elementList, restArgs) => {
      ...
      while (i < elementList.length) {
        ...
        for (let r = 0; r < ruleList.length; r++) {
          ...
          this.highlightMatchResult.push(...searchResult.map((result) => __spreadProps(__spreadValues(__spreadValues(__spreadValues({}, result), rule), restArgs), {
...
class SelectControl {
  ...
  keydown(evt) {
    ...
    const startElement = elementList[startIndex];
    const endElement = elementList[endIndex];
    const inputAble = (_b = (_a = this.element.control) == null ? void 0 : _a.selectExclusiveOptions) == null ? void 0 : _b.inputAble;
    if (evt.key === KeyMap.Backspace) {
      if (startIndex !== endIndex) {
        if (inputAble) {
  ...
  awake() {
    var _a;
    if (this.isPopup || this.control.getIsDisabledControl() || !this.control.getIsRangeWithinControl()) {
      return;
    }
    const { startIndex } = this.control.getRange();
    const elementList = this.control.getElementList();
...
class NumberControl extends TextControl {
  setValue(data2, context = {}, options = {}) {
    ...
    const range = context.range || this.control.getRange();
    this.control.shrinkBoundary(context);
    const controlElementList = deepClone(data2);
    const { startIndex, endIndex } = range;
    const startElement = elementList[startIndex];
    if (this.control.getIsExistValueByElementListIndex(elementList, startIndex)) {
      let preIndex = startIndex;
      while (preIndex > 0) {
        const preElement = elementList[preIndex];
        if (preElement.controlId !== startElement.controlId || preElement.controlComponent === ControlComponent.PREFIX || preElement.controlComponent === ControlComponent.PRE_TEXT) {
    ...
    return super.setValue(data2, context, options);
  }
}
class Control {
  constructor(draw) {
  ...
  getIsExistValueByElementListIndex(elementList, index2) {
    var _a, _b, _c;
    const element = elementList[index2];
    if (!element.controlId)
      return false;
    if (((_a = element.control) == null ? void 0 : _a.type) === ControlType.CHECKBOX || ((_b = element.control) == null ? void 0 : _b.type) === ControlType.RADIO) {
      return !!((_c = element.control) == null ? void 0 : _c.code);
    }
    if (element.controlComponent === ControlComponent.VALUE) {
      return true;
    }
    ...
    if (element.controlComponent === ControlComponent.PREFIX || element.controlComponent === ControlComponent.PRE_TEXT) {
      let i = index2 + 1;
      while (i < elementList.length) {
    ...
    if (element.controlComponent === ControlComponent.POSTFIX || element.controlComponent === ControlComponent.POST_TEXT) {
      let i = index2 - 1;
      while (i >= 0) {
  ...
  getControlHighlight(elementList, index2) {
    return this.controlSearch.getControlHighlight(elementList, index2);
  }
  getContainer() {
  ...
  initControl() {
    ...
    if (this.activeControl) {
      ...
      if (((_a = this.preElement) == null ? void 0 : _a.controlId) === element.controlId) {
        if (element.controlComponent === ControlComponent.POSTFIX) {
          this.emitControlChange(ControlState.INACTIVE);
        } else if (((_b = this.preElement) == null ? void 0 : _b.controlComponent) === ControlComponent.POSTFIX) {
  ...
  destroyControl(options = {}) {
    ...
    if (isEmitEvent && ((_a = this.preElement) == null ? void 0 : _a.controlComponent) !== ControlComponent.POSTFIX) {
      this.emitControlChange(ControlState.INACTIVE);
    }
    this.preElement = null;
    this.activeControl = null;
  ...
  selectValue() {
    ...
    const startElement = elementList[startIndex];
    if (!(startElement == null ? void 0 : startElement.controlId) || startElement.controlComponent !== ControlComponent.VALUE && ((_a = elementList[startIndex + 1]) == null ? void 0 : _a.controlComponent) === ControlComponent.VALUE) {
      return false;
    }
    let preIndex = startIndex;
    while (preIndex > 0) {
    ...
    if (preIndex !== nextIndex) {
      const range = this.range.getRange();
      this.range.replaceRange(__spreadProps(__spreadValues({}, range), {
        startIndex: preIndex,
        endIndex: nextIndex
      }));
      this.draw.render({
  ...
  removeControl(startIndex, context = {}) {
    ...
    if (!this.draw.isDesignMode() && !(startElement == null ? void 0 : startElement.hide) && !((_a = startElement == null ? void 0 : startElement.control) == null ? void 0 : _a.hide) && !((_b = startElement == null ? void 0 : startElement.area) == null ? void 0 : _b.hide)) {
      ...
      if (mode === EditorMode.FORM && this.options.modeRule[mode].controlDeletableDisabled) {
        return null;
      }
    }
    let leftIndex = -1;
  ...
  getValueById(payload) {
    ...
    const getValue = (elementList, zone2) => {
      ...
      while (i < elementList.length) {
        ...
        if (!element.control || groupId && element.control.groupId !== groupId || id && element.controlId !== id || conceptId && element.control.conceptId !== conceptId || areaId && element.areaId !== areaId) {
          continue;
        }
    ...
    const setValue = (elementList) => {
      ...
      while (i < elementList.length) {
        ...
        const payloadItem = payload.find((p) => {
          var _a;
          return (!p.groupId || p.groupId === ((_a = element.control) == null ? void 0 : _a.groupId)) && (p.id && element.controlId === p.id || p.conceptId && element.control.conceptId === p.conceptId || p.areaId && element.areaId === p.areaId);
        });
        if (!payloadItem)
          continue;
...
const encodedJs$1 = "";
...
class WorkerManager {
  constructor(draw) {
    ...
    __publicField(this, "valueWorker");
    this.draw = draw;
...
class Previewer {
  constructor(draw) {
    ...
    __publicField(this, "curShowElement");
    __publicField(this, "imageCount");
    __publicField(this, "imagePre");
    __publicField(this, "imageNext");
  ...
  _drawPreviewer() {
    ...
    imagePre.onclick = () => {
      const curIndex = this.imageList.findIndex((el) => {
        var _a;
        return el.id === ((_a = this.curShowElement) == null ? void 0 : _a.id);
      });
      if (curIndex <= 0)
        return;
  ...
  _updateImageNavigate() {
    const currentIndex = this.imageList.findIndex((el) => {
      var _a;
      return el.id === ((_a = this.curShowElement) == null ? void 0 : _a.id);
    });
    this.imageCount.innerText = `${currentIndex + 1} / ${this.imageList.length}`;
    if (currentIndex <= 0) {
  ...
  render() {
    var _a, _b;
    const mode = this.draw.getMode();
    if (!this.curElement || this.curElement.imgToolDisabled && !this.draw.isDesignMode() || mode === EditorMode.PRINT && ((_a = this.options.modeRule[EditorMode.PRINT]) == null ? void 0 : _a.imagePreviewerDisabled) || mode === EditorMode.READONLY && ((_b = this.options.modeRule[EditorMode.READONLY]) == null ? void 0 : _b.imagePreviewerDisabled)) {
      return;
    }
  ...
  drawResizer(element, position = null, options = {}) {
    var _a, _b;
    const mode = this.draw.getMode();
    if (element.imgToolDisabled && !this.draw.isDesignMode() || mode === EditorMode.PRINT && ((_a = this.options.modeRule[EditorMode.PRINT]) == null ? void 0 : _a.imagePreviewerDisabled) || mode === EditorMode.READONLY && ((_b = this.options.modeRule[EditorMode.READONLY]) == null ? void 0 : _b.imagePreviewerDisabled)) {
      return;
    }
...
class VideoBlock {
  ...
  snapshot(ctx, x, y) {
    return new Promise((resolve, reject) => {
      var _a, _b;
      const src = ((_b = (_a = this.element.block) == null ? void 0 : _a.videoBlock) == null ? void 0 : _b.src) || "";
      if (this.videoCache.has(src)) {
        const video = this.videoCache.get(src);
        ctx.drawImage(video, x, y, this.element.metrics.width, this.element.metrics.height);
...
class BaseBlock {
  constructor(blockParticle, element) {
    ...
    __publicField(this, "mousedownY");
    __publicField(this, "curHandleIndex");
    __publicField(this, "isAllowResize");
    this.draw = blockParticle.getDraw();
    this.options = this.draw.getOptions();
    this.blockContainer = blockParticle.getBlockContainer();
    this.element = element;
    this.block = null;
    const { blockItem, resizerMask, resizerSelection, resizerHandleList } = this._createBlockItem();
  ...
  snapshot(ctx, x, y) {
    ...
    if (block.type === BlockType.VIDEO) {
      ...
      if (this.blockCache.has(this.element.id)) {
        const videoBlock = this.blockCache.get(this.element.id);
...
class ZoneTip {
  constructor(draw, zone2) {
    ...
    __publicField(this, "container");
    __publicField(this, "pageContainer");
    __publicField(this, "isDisableMouseMove");
...
class Footer {
  ...
  render(ctx, pageNo) {
    ctx.save();
    ctx.globalAlpha = this.zone.isFooterActive() ? 1 : this.options.footer.inactiveAlpha;
    const innerWidth = this.draw.getInnerWidth();
    const maxHeight = this.getMaxHeight();
    const rowList = [];
    let curRowHeight = 0;
...
class WhiteSpaceParticle {
  ...
  render(ctx, element, x, y) {
    const { scale, whiteSpace: { color, radius } } = this.options;
    const metrics = element.metrics;
    ctx.save();
    ctx.fillStyle = color;
    ctx.beginPath();
...
class Area {
  ...
  render(ctx, pageNo) {
    ...
    for (const areaInfoItem of this.areaInfoMap) {
      ...
      if ((area == null ? void 0 : area.hide) || !(area == null ? void 0 : area.backgroundColor) && !(area == null ? void 0 : area.borderColor) && !(area == null ? void 0 : area.placeholder)) {
        continue;
      }
  ...
  setAreaValue(payload) {
    ...
    this.draw.spliceElementList(elementList, positionList[0].index, positionList.length, valueList, {
      isIgnoreDeletedRule: true
    });
    this.draw.render({
      isSetCursor: false
...
class Graffiti {
  ...
  drawing(evt) {
    ...
    const DISTANCE = 2;
    if (this.startStroke && Math.abs(this.startStroke.points[0] - offsetX) < DISTANCE && Math.abs(this.startStroke.points[1] - offsetY) < DISTANCE) {
      return;
    }
    ...
    if (!((_a = currentValue == null ? void 0 : currentValue.strokes) == null ? void 0 : _a.length))
      return;
    const { scale } = this.options;
    const lastPoints = currentValue.strokes[currentValue.strokes.length - 1].points;
...
class Draw {
  constructor(rootContainer, options, data2, listener, eventBus, override) {
    ...
    __publicField(this, "whiteSpaceParticle");
    __publicField(this, "control");
  ...
  isReadonly() {
    var _a, _b;
    if ((_b = (_a = this.area.getActiveAreaInfo()) == null ? void 0 : _a.area) == null ? void 0 : _b.mode) {
  ...
  spliceElementList(elementList, start, deleteCount, items, options) {
    ...
    const { group: group2, modeRule } = this.options;
    if (deleteCount > 0) {
    ...
    for (let i = 0; i < elementList.length; i++) {
      ...
      if ((element.hide || ((_d = element.control) == null ? void 0 : _d.hide) || ((_e = element.area) == null ? void 0 : _e.hide)) && !this.isDesignMode()) {
        ...
        if (isPagingMode) {
          ...
          if (curPagePreHeight + firstTrHeight + rowMarginHeight > height2 || element.pagingIndex !== 0 && element.trList[0].pagingRepeat || ((_i = elementList[i - 1]) == null ? void 0 : _i.type) === ElementType.PAGE_BREAK) {
        ...
        metrics.boundingBoxDescent = 0;
        metrics.boundingBoxAscent = this.textParticle.getBasisWordBoundingBoxAscent(ctx, ctx.font);
      } else if (element.type === ElementType.BLOCK) {
        if (!element.width) {
          metrics.width = availableWidth;
        } else {
  ...
  _drawHighlight(ctx, payload) {
    ...
    for (let i = 0; i < rowList.length; i++) {
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        const highlight = element.highlight || this.control.getControlHighlight(elementList, curRow.startIndex + j);
        if (highlight) {
          ...
          this.highlight.recordFillInfo(ctx, x - offsetX, y + marginHeight - highlightMarginHeight, element.metrics.width + offsetX, curRow.height - 2 * marginHeight + 2 * highlightMarginHeight, highlight);
  ...
  drawRow(ctx, payload) {
    ...
    const { scale, table: { tdPadding }, group: group2, lineBreak, whiteSpace } = this.options;
    const { rowList, pageNo, elementList, positionList, startIndex, zone: zone2, isDrawLineBreak = !lineBreak.disabled, isDrawWhiteSpace = !whiteSpace.disabled } = payload;
    ...
    for (let i = 0; i < rowList.length; i++) {
      ...
      for (let j = 0; j < curRow.elementList.length; j++) {
        ...
        if ((_c = element.control) == null ? void 0 : _c.border) {
          if (((_d = preElement == null ? void 0 : preElement.control) == null ? void 0 : _d.border) && preElement.controlId !== element.controlId) {
        ...
        } else if ((_e = preElement == null ? void 0 : preElement.control) == null ? void 0 : _e.border) {
          this.control.drawBorder(ctx);
        }
        if (element.underline || ((_f = element.control) == null ? void 0 : _f.underline)) {
          ...
          const color = ((_g = element.control) == null ? void 0 : _g.underline) ? this.options.underlineColor : element.color;
          this.underline.recordFillInfo(ctx, x - offsetX, y + curRow.height - rowMargin + offsetY2, metrics.width + offsetX, 0, color, (_h = element.textDecoration) == null ? void 0 : _h.style);
        } else if ((preElement == null ? void 0 : preElement.underline) || ((_i = preElement == null ? void 0 : preElement.control) == null ? void 0 : _i.underline)) {
          this.underline.render(ctx);
        }
        if (element.strikeout) {
  ...
  _drawPage(payload) {
    ...
    if (!isPrintMode || !((_a = this.options.modeRule[EditorMode.PRINT]) == null ? void 0 : _a.backgroundDisabled)) {
    ...
    const index2 = (_b = rowList[0]) == null ? void 0 : _b.startIndex;
    this.drawRow(ctx, {
      elementList,
      positionList,
      rowList,
    ...
    if (this.elementList.length <= 1 && !((_c = this.elementList[0]) == null ? void 0 : _c.listId)) {
...
class Command {
  constructor(adapt) {
    ...
    __publicField(this, "executeTranslate");
    ...
    __publicField(this, "executeSetControlValueList");
    __publicField(this, "executeSetControlExtension");
    __publicField(this, "executeSetControlExtensionList");
    __publicField(this, "executeSetControlProperties");
    __publicField(this, "executeSetControlPropertiesList");
    ...
    __publicField(this, "executeJumpControl");
    ...
    __publicField(this, "getValueAsync");
    ...
    this.executeSetControlExtensionList = adapt.setControlExtensionList.bind(adapt);
    this.executeSetControlProperties = adapt.setControlProperties.bind(adapt);
    this.executeSetControlPropertiesList = adapt.setControlPropertiesList.bind(adapt);
...
class CommandAdapt {
  ...
  hyperlink(payload) {
    ...
    if (!url || !(valueList == null ? void 0 : valueList.length))
      return;
    const isDisabled = this.draw.isReadonly() || this.draw.isDisabled();
    if (isDisabled)
      return;
  ...
  getRangeContext() {
    ...
    if (!this.draw.getCursor().getHitLineStartIndex()) {
      startColNo = ((_a = startRow.elementList[0]) == null ? void 0 : _a.value) === ZERO ? startPosition.index - startRow.startIndex : startPosition.index - startRow.startIndex + 1;
    }
    if (startPosition === endPosition) {
      endColNo = startColNo;
    } else {
      endColNo = ((_b = endRow.elementList[0]) == null ? void 0 : _b.value) === ZERO ? endPosition.index - endRow.startIndex : endPosition.index - endRow.startIndex + 1;
    }
    const rangeRects = [];
  ...
  jumpControl(payload) {
    this.draw.getControl().initNextControl({
      direction: payload == null ? void 0 : payload.direction
    });
  }
  getContainer() {
    return this.draw.getContainer();
  ...
  getTitleValue(payload) {
    ...
    const getValue = (elementList, zone2) => {
      ...
      while (i < elementList.length) {
        ...
        result.push(__spreadProps(__spreadValues({}, element.title), {
  ...
  focus(payload) {
    var _a, _b;
    const { position = LocationPosition.AFTER, isMoveCursorToVisible = true, rowNo, range } = payload || {};
    let curIndex = -1;
    if (range) {
      this.range.replaceRange(range);
      curIndex = position === LocationPosition.BEFORE ? range.startIndex : range.endIndex;
    } else if (isNumber(rowNo)) {
      const rowList = this.draw.getOriginalRowList();
      curIndex = position === LocationPosition.BEFORE ? (_a = rowList[rowNo]) == null ? void 0 : _a.startIndex : ((_b = rowList[rowNo + 1]) == null ? void 0 : _b.startIndex) - 1;
      if (!isNumber(curIndex))
        return;
      this.range.setRange(curIndex, curIndex);
    } else {
      curIndex = position === LocationPosition.BEFORE ? 0 : this.draw.getOriginalMainElementList().length - 1;
      this.range.setRange(curIndex, curIndex);
    }
    const renderParams = {
      isCompute: false,
...
class Listener {
  constructor() {
    ...
    __publicField(this, "controlContentChange");
...
const hyperlinkMenus = [
  ...
  },
  {
    ...
    callback: (command, context) => {
      var _a;
      const url = window.prompt(command.executeTranslate("contextmenu.hyperlink.edit"), (_a = context.startElement) == null ? void 0 : _a.url);
      if (url) {
